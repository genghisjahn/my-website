...
author:
  name: Jon Wear
summary: Added some updates I've been wanting to the deploy pipeline as features I thought were missing.
tags:
  - name: web
    slug: web
hero:
...
For a while I was just making images from ChatGPT and putting the PNGs as hero images at the top of the articles.  That worked fine but those images are huge, so as part of the deploy process now I convert all .png, .jpg, and other image files to [.webp](https://en.wikipedia.org/wiki/WebP) using [cwebp](https://developers.google.com/speed/webp/docs/cwebp).  That really helps a lot with image size.  For example: 
